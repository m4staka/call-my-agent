...
A small Go CLI that polls a Telegram bot, forwards allowed chats to the Codex CLI, and replies with the model output while maintaining per-chat sessions. The CLI also supports manual heartbeats.
...
- **Session store**: conversations are serialized to `sessionStorePath` after each change (defaults to `~/.cma/sessions.json`), so `start` and `heartbeat` share the same context.
...
`start` runs the polling loop and `heartbeat` runs a single heartbeat pass.
